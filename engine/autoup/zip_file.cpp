...
zip_info zip_file::getinfo(int index)
{
	...
	result.is_dir = mz_zip_reader_is_file_a_directory(archive_.get(), static_cast<mz_uint>(index));
...
void zip_file::extract(const zip_info &member, const std::string &path)
{
	if (member.is_dir)
	{
		std::tr2::sys::path path_(path);
		path_ /= member.filename.c_str();
		std::tr2::sys::create_directory(path_);
	}
	else
	{
